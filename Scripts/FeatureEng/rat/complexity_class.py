...
class ComplexClass:
    ...
    def calculate_hurst_exp(self, channel_data):
        for epoch in channel_data:
            hurst_values = compute_hurst_exp(np.expand_dims(epoch, axis=0))
        return np.array(hurst_values).flatten()

    def calculate_hfd_exp(self, channel_data):
        for epoch in channel_data:
            hfd_values = compute_higuchi_fd(np.expand_dims(epoch, axis=0))
        return np.array(hfd_values).flatten()

    def transform_to_dataframe(self, complexity_array, channel_labels):
        ...
        'Epoch': epochs,
        'Channel': channels,
        f'{complexity_array}': complex_values
        })
